...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    ...
    function polymuse_add_model_and_thumbnail_to_gallery($html, $attachment_id) {
        ...
        if (!empty($model_url)) {
            ...
            if ($first_image) {
                ...
                $model_viewer .= 'class="woocommerce-product-gallery__image polymuse-model-viewer no-swipe" '; // Changed: Added no-swipe class
                $model_viewer .= 'style="position: relative; overflow: hidden;">'; // Changed: Added inline styles
    ...
    add_action('wp_enqueue_scripts', 'polymuse_enqueue_assets', 20); // Changed: Increased priority

    // New: Disable WooCommerce gallery slider functionality
    add_action('wp', 'polymuse_disable_gallery_slider', 100);
    ...
    // New: Remove slider classes
    function polymuse_remove_slider_classes($classes) {
        $classes = array_diff($classes, array(
            'woocommerce-product-gallery--with-images',
            'woocommerce-product-gallery--columns-4',
            'images'
        ));
        $classes[] = 'polymuse-no-slider';
        return $classes;
    }

    // New: Add CSS to disable swiping
    function polymuse_disable_swipe_css() {
        wp_enqueue_style('polymuse-styles', plugins_url('styles.css', __FILE__));
        $custom_css = "
            ...
            .polymuse-no-slider .woocommerce-product-gallery__image {
                float: none !important;
                width: 100% !important;
                margin: 0 !important;
            }
            .polymuse-model-viewer {
                touch-action: pan-y !important; /* Allows vertical scroll but prevents horizontal swipe */
        ...
        ";
        wp_add_inline_style('polymuse-styles', $custom_css);
    }
}